...
def adbExtract():
    ...
    if os.path.isfile("../android_backup/backup.ab") and os.path.getsize("../android_backup/backup.ab") >= 1:
        ...
        subprocess.call(["java", "-jar", "./MobileSQLiteAnalysis/platform-tools/abe.jar", "unpack", "../android_backup/backup.ab", "../android_backup/backup.tar"])
        if os.path.isfile("../android_backup/backup.tar"):
            print(dt, ": .ab Successfully Converted to .tar")
            # Extract Tar File to RawDump

            subprocess.call(["7za", "x", "../android_backup/backup.tar", "-o../rawdump", "-aou"], cwd=working_dir)
        else:
